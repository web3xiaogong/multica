...
function useFormatRelativeDate() {
  const t = useViewTranslations("autopilot");
  return (date: string) => {
    const diff = Date.now() - new Date(date).getTime();
    const days = Math.floor(diff / (1000 * 60 * 60 * 24));
    if (days < 1) return t("relativeDate.today");
    if (days === 1) return t("relativeDate.oneDayAgo");
    if (days < 30) return t("relativeDate.daysAgo", { days });
    const months = Math.floor(days / 30);
    return t("relativeDate.monthsAgo", { months });
  };
}

const STATUS_KEY_TO_COLOR: Record<string, { color: string; icon: typeof Zap }> = {
...
function AutopilotRow({ autopilot }: { autopilot: Autopilot }) {
  ...
  const statusKey = STATUS_KEY_TO_COLOR[autopilot.status] ? autopilot.status : "active";
  const statusCfg = STATUS_KEY_TO_COLOR[statusKey]!;
    ...
    <div className="group/row flex h-11 items-center gap-2 px-5 text-sm transition-colors hover:bg-accent/40">
      ...
      <span className="w-24 shrink-0 text-center text-xs text-muted-foreground">
        {autopilot.execution_mode === "create_issue"
          ? t("executionMode.createIssue")
          : autopilot.execution_mode === "run_only"
            ? t("executionMode.runOnly")
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="sticky top-0 z-[1] flex h-8 items-center gap-2 border-b bg-muted/30 px-5 text-xs font-medium text-muted-foreground">
              ...
              <span className="w-32 shrink-0">{t("columns.agent")}</span>
              <span className="w-24 text-center shrink-0">{t("columns.mode")}</span>
              <span className="w-20 text-center shrink-0">{t("columns.status")}</span>
              <span className="w-20 text-right shrink-0">{t("columns.lastRun")}</span>
            </div>
