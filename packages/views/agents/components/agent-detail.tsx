...
export function AgentDetail({
  ...
  const t = useViewTranslations("agents.detail");
    ...
    <div className="flex h-full flex-col">
      ...
      {isArchived && (
        <div className="flex items-center gap-2 bg-muted/50 px-4 py-2 text-xs text-muted-foreground border-b">
          ...
          <span className="flex-1">{t("banner.archived")}</span>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <span className="flex items-center gap-1 rounded-md bg-muted px-1.5 py-0.5 text-xs font-medium text-muted-foreground">
              ...
              {runtimeDevice?.name ?? (agent.runtime_mode === "cloud" ? t("runtime.cloud") : t("runtime.local"))}
            </span>
          </div>
        </div>
        {!isArchived && (
          ...
          <DialogContent className="max-w-sm" showCloseButton={false}>
            <div className="flex items-center gap-3">
              ...
              <DialogHeader className="flex-1 gap-1">
                <DialogTitle className="text-sm font-semibold">{t("dialog.title")}</DialogTitle>
                <DialogDescription className="text-xs">
                  {t("dialog.description", { name: agent.name })}
                </DialogDescription>
              </DialogHeader>
            </div>
            <DialogFooter>
